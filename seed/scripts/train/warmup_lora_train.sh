...
source ./seed/scripts/train/base_training_args.sh
...
if [[ $model_path == "./checkpoints/Llama-2-13b-hf" ]]; then
    base_training_args="$base_training_args --fsdp 'full_shard auto_wrap' --fsdp_config llama2_13b_finetune"
    elif [[ $model_path == "./checkpoints/Llama-2-70b-hf" ]]; then
        base_training_args="$base_training_args --fsdp 'full_shard auto_wrap' --fsdp_config llama2_70b_finetune"
