...
def obtain_on_policy_distillation_gradients(student_model, teacher_model, input_ids):
    """
    Compute gradients for per-token on-policy distillation (KL loss) and return as a vector.

    Args:
        student_model: nn.Module, 学生模型
        teacher_model: nn.Module, 教师模型（不训练）
        input_ids: Tensor, 输入 batch (B, seq_len)

    Returns:
        vectorized_grad: 展平的梯度向量
    """
    # 1. Forward pass
    student_model.train()
    teacher_model.eval()    
    ...
    vectorized_grad = torch.cat(
        [p.grad.view(-1) for p in student_model.parameters() if p.grad is not None]
    )
...
def prepare_optimizer_state(model, optimizer_state, device):
    ...
    names = [n for n in range(len(names))]
...
def collect_grads(dataloader,
    ...
    def _project(current_full_grads, projected_grads):
        ...
        current_full_grads_fp32 = torch.stack(current_full_grads).float()
        for i, projector in enumerate(projectors):
            current_projected_grads = projector.project(
                current_full_grads_fp32, model_id=model_id)
            # print(current_projected_grads.abs().mean(dim=0))
