...
def main(args):
    ...
    if args.model_name_or_path:
        ...
        outputs = [output.strip().replace("</s>", "") for output in outputs]
    ...
    metric = evaluate.load("/mnt/bn/bes-mllm-shared/zhangyuan.kevin/SEED/evaluation/evaluate/metrics/squad/squad.py")

    def convert_answers(answers):
        """
        answers: List[{"text": str, "answer_start": int}]
        return:  {"text": List[str], "answer_start": List[int]}
        """
        return {
    ...
    for lang in data_languages:
        ...
        lang_references = [{"id": example["id"], "answers": convert_answers(example["answers"])}
